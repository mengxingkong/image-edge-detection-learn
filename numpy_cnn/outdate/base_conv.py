...
class Conv2D(object):
    def __init__(self, shape, output_channels, ksize=3, stride=1, method='VALID'):
        ...
        print((shape[1] - ksize + 1) / self.stride)
    ...
    def gradient(self, eta):
        ...
        for i in range(self.batchsize):
            # 这里不应该是 卷积层的输出吗 col_iamge 好像是 卷基层的输入 ????   解决了，这里求的是 卷积核的 偏导
            test_a = np.dot(self.col_image[i].T, col_eta[i])
        ...
        if self.method == 'VALID':
            # (a,b) 表示在 哪一个轴 前面填充 a 个 0， 后面填充 b个0，（0，0）表示不填充该维度
