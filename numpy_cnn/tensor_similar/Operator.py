import numpy as np
from numpy_cnn.tensor_similar.Variable import Variable, GLOBAL_VARIABLE_SCOPE
...
class Conv2D(Operator):
    def __init__(self, kernel_shape=list, input_variable=Variable, name=str, stride=1, padding='SAME'):
        ...
        if self.padding == 'SAME':
            _output_shape = [self.batch_size, input_variable.shape[1] / stride, input_variable.shape[2] / stride, self.output_num]

        if self.padding == 'VALID':
            _output_shape= [self.batch_size, (input_variable.shape[1] - self.ksize) / stride + 1, (input_variable.shape[2] - self.ksize) / stride + 1, self.output_num]

        self.output_variables = Variable(_output_shape, name='out', scope=name)
        self.input_variables = input_variable
        Operator.__init__(self, name, self.input_variables, self.input_variables)
    ...
    def _conv(self, inpput=Variable, output=Variable, weights=np.ndarray, bias=np.ndarray):
        col_weights = weights.reshape(-1, self.output_num)

        if self.padding == 'SAME':
            #  为啥是 self.ksize / 2  参考 卷积的三种模式的理解 full，same，valid
            batch_img = np.pad(inpput.data, ((0, 0), (self.ksize // 2, self.ksize // 2), (self.ksize // 2, self.ksize // 2), (0, 0)), 'constant', constant_values=0)

        else:
            batch_img = inpput.data
...
class MaxPooling(Operator):
    def __init__(self, ksize=2, input_variable=Variable, name=str, stride=2):
        ...
        _output_shape = [self.batch_size, input_variable.shape[1] // stride, input_variable.shape[2] // stride, self.output_channels]
            ...
            for c in range(self.output_channels):
                for i in range(0, self.input_variables.shape[1], self.stride):
                    for j in range(0, self.input_variables.shape[2], self.stride):
                        _out[b, i//self.stride, j//self.stride, c] = np.max(self.input_variables.data[b, i:i + self.ksize, j:j + self.ksize, c])
...
class FullyConnect(Operator):
    def __init__(self, output_num, input_variable=Variable, name=str):
        ...
        self.output__variables = Variable([self.batch_size, self.output_num], name='out',scope=name)
        self.input_variables = input_variable
        Operator.__init__(self, name, self.input_variables, self.output_variables)
...
class SoftmaxLoss(Operator):
    def __init__(self, predict = Variable, label=Variable, name = str):
        ...
        self.prediction = Variable(predict.shape, name='prediciton', scope=name)
    ...
    def forward(self):
        if self.wait_forward:
            ...
            lable = self.input_variables[1].data

            self.prediction.data = self.predict(predict)

            self.loss.data =0
            for i in range(self.batch_size):
                self.loss.data += np.log(np.sum(np.exp(predict[i]))) - predict[i, lable[i]]
            self.wait_forward = False
            return
        else:
            pass
    ...
    def backward(self):
        if self.wait_forward:
            ...
            # 这块不理解
            for i in range(self.batch_size):
                self.input_variables[0].diff[i, self.input_variables[1].data[i]] -= i
            self.wait_forward = True
            return
...
class DropOut(Operator):
    def __index__(self, name, phase, input_variable=Variable, prob=0.5):
        self.input_variables = input_variable
        self.output_variable = Variable(shape=input_variable.shape, scope=name, name='out')
        self.prob = prob
        self.phase = phase
    ...
    def forward(self):
        if self.wait_forward:
            ...
            if self.phase == 'train':
                ...
                self.output_variable.data = self.input_variables.data * self.index
                # 这一行 不立即
                self.output_variable.data /= self.prob
            elif self.phase== 'test':
                self.output_variable.data = self.input_variables.data
            else:
...
def im2col(img, ksize, stride):
    ...
    for i in range(0, img.shape[1] - ksize + 1, stride):
        for j in range(0, img.shape[2] - ksize + 1, stride):
            col = img[:, i:i+ksize, j:j+ksize, :].reshape[-1]
